...
from financeAnalysis.backend.portfolioManagement import getPortfolio_ml
...
def compile_data_to_columns(date=datetime.today()):
    ...
    if 'updated_date' in main_df.columns:
        main_df.drop(['updated_date'], axis='columns', inplace=True)
    return main_df
...
def do_ml(date=datetime.today()):
    df = compile_data_to_columns(date)
    for ticker in df.columns:
        ...
        StockTickerHistory.objects.update_or_create(symbol_id=ticker, updated_on=date,
